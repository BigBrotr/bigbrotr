...
Multi-CCD CPUs (Ryzen 9, EPYC, Threadripper) benefit from NUMA awareness to distribute workload across chiplets:
...
**Create per-deployment work directory structure**:
...
mv BigBrotr-bigbrotr-*/deployments/bigbrotr bigbrotr-production
rm -rf BigBrotr-bigbrotr-*
```

### 5.2 — Configure Docker Hub Images

Edit `docker-compose.yaml` to use pre-built images from Docker Hub instead of building locally. Replace every `build:` block in the 9 service definitions with an `image:` line:
...
```bash
# Create per-deployment PostgreSQL data directory on the RAID10 disk
...
### 5.3.1 — Link Dumps to Work Disk

Symlink the dumps directory to the per-deployment workpool folder:

```bash
ln -s /mnt/work/bigbrotr-production/dumps /opt/bigbrotr-production/dumps
```

### 5.3.2 — Fix Permissions for Monitor GeoLite2 Downloads
...
NOSTR_PRIVATE_KEY_GLOBAL_PAGERANK_V1=${NOSTR_ASSERTOR_KEY}
...
A `backup.sh` script is included in the deployment folder. It dumps the database to the `dumps/` directory (symlinked to `/mnt/work/bigbrotr-production/dumps/`), compressed with gzip, and keeps only the 7 most recent dumps.

```bash
/opt/bigbrotr-production/backup.sh
```

To schedule automatic daily backups (optional):
...
```bash
/opt/bigbrotr-production/backup.sh
# Dumps saved to ./dumps/ (symlinked to /mnt/work/bigbrotr-production/dumps/)
```
...
| `/opt/bigbrotr-production/data/` | PostgreSQL data (symlink to `/mnt/pgdata/bigbrotr-production/`) |
| `/opt/bigbrotr-production/dumps/` | Database backups (symlink to `/mnt/work/bigbrotr-production/dumps/`) |
| `/mnt/pgdata/bigbrotr-production/` | PostgreSQL data on RAID10 |
| `/mnt/work/bigbrotr-production/` | Dumps, exports, analysis on workpool |

---

## Troubleshooting

### IPv6 DNS resolution in containers

**Symptom**: `connect_failed` with `"Temporary failure in name resolution"` for relays reachable from the host.

**Cause**: glibc prefers IPv6 (RFC 6724). Some relays return AAAA records but the container has no IPv6 connectivity.

**Fix**: The deployment includes `gai.conf` which forces IPv4 precedence. Verify it's mounted:
