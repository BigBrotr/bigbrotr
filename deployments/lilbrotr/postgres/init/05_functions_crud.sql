 ...
 * the same (owner, state_type, state_key) already exists, its
 * state_value is fully replaced. DISTINCT ON deduplicates within the batch.
 *
 * Parameters:
 *   p_owners          - Array of state owner identifiers
 *   p_state_types     - Array of state categories
 *   p_state_keys      - Array of unique keys within each owner+type
...
BEGIN
    ...
    SELECT DISTINCT ON (owner, state_type, state_key)
    ...
    FROM unnest(
        ...
        p_state_keys,
        p_state_values
    ) AS t(owner, state_type, state_key, state_value)
    ON CONFLICT (owner, state_type, state_key)
    DO UPDATE SET
 ...
 * owner and state type, ordered by state_key ascending.
 *
 * Parameters:
 *   p_owner       - State owner identifier
...
BEGIN
    ...
    USING unnest(
        ...
        p_state_types,
        p_state_keys
    ) AS d(owner, state_type, state_key)
    WHERE ss.owner = d.owner
      AND ss.state_type = d.state_type
      AND ss.state_key = d.state_key;
