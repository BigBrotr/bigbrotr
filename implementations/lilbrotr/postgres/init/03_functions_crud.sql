 ...
 *
 * LilBrotr difference: events_insert() accepts the same parameters as
 * BigBrotr for interface compatibility, but computes tagvalues from tags
 * and discards tags, content, and sig (not stored in this schema).
 *
 ...
 * signature as BigBrotr for interface compatibility, but only stores id,
 * pubkey, created_at, kind, and tagvalues (computed from tags). The tags,
 * contents, and sigs arrays are accepted but not persisted.
 *
 * Parameters:
 ...
 *   p_tags         - Array of JSONB tag arrays (used to compute tagvalues, not stored)
 *   p_contents     - Array of event content strings (accepted but not stored)
 *   p_sigs         - Array of 64-byte Schnorr signatures (accepted but not stored)
 *
 * Returns: Number of newly inserted rows
 */
CREATE OR REPLACE FUNCTION events_insert(
    ...
    p_contents TEXT[],        -- Accepted but NOT stored
    p_sigs BYTEA[]            -- Accepted but NOT stored
)
RETURNS INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
    row_count INTEGER;
BEGIN
    -- Store only essential fields; compute tagvalues from tags, discard the rest
...
'Bulk insert events (lightweight: computes tagvalues, discards tags/content/sig)';
 ...
 *
 * Parameters:
 *   p_event_ids   - Array of event hashes (must exist in events table)
 *   p_relay_urls  - Array of relay URLs (must exist in relays table)
 ...
 *   p_metadata_values  - Array of JSON values (unused, kept for interface compatibility)
 *   p_metadata_types   - Array of check types (nip11_fetch, nip66_rtt, etc.)
 ...
 * so the lightweight storage behavior of events_insert() applies here too.
 *
...
BEGIN
    ...
    -- Ensure event records exist (lightweight: computes tagvalues, discards rest)
