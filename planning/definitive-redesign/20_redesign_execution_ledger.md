...
| 10. Final cleanup, rename sweep, and closeout audit | done | The residual-name sweep, compatibility-cruft removal, final design-hygiene audit, and release-readiness verification gate are all closed; the redesign execution program is now complete |
...
| 9.2 MkDocs information architecture and narrative rewrite | done | `docs: rewrite mkdocs information architecture` | Reworked the MkDocs site around the final project story instead of the pre-redesign one: `mkdocs.yml` now exposes `Read Side` and `Deployments` as first-class user-guide pages; the top-level landing pages for `docs/`, `getting-started/`, `user-guide/`, `how-to/`, and `development/` were rewritten to frame BigBrotr as a storage-first observability system with independent services, protocol-agnostic public-read adapters, and deployment profiles; new narrative pages `docs/user-guide/read-side.md` and `docs/user-guide/deployments.md` explain the shared `ReadCore`, readable-resource contract, protocol exposure policy, deployment folder contract, and storage profiles; and the most visible snippets and user-guide sections were realigned so Api/Dvm, quickstart, and service docs no longer narrate the old raw-schema/read-model mental model as if it were the core architecture. Targeted markdownlint and codespell passed, `mkdocs build --strict` passed, then full `make ci` (`3319 passed`) and `uv lock --check` passed before closure |
| 9.3 Folder-level `README.md` and local guidance rewrite | done | `docs: add folder-level repository readmes` | Added a coherent local README layer across the maintained tracked repository surface so a contributor can orient themselves folder-by-folder without reverse-engineering the whole tree first. The slice introduced README coverage for `.github/` and its maintained subfolders, `planning/`, `src/` plus the tracked Python package and service packages, `tests/` plus the maintained unit/integration subfolders, `tools/` plus the tracked SQL-template tree, and the non-page local docs folders inside `docs/`. The README set is intentionally human-oriented and concise: each file explains what the folder is for, what lives there, and the local rules that matter. The audit also made the explicit exception policy concrete: generated or cache folders such as `__pycache__`, deployment runtime state, `src/bigbrotr.egg-info`, and locally ignored non-repository folders are not treated as maintained documentation surfaces, while MkDocs section folders that already use `index.md` as their human entrypoint (`docs/`, `docs/development/`, `docs/getting-started/`, `docs/how-to/`, `docs/user-guide/`) are treated as covered by that local page instead of carrying a conflicting `README.md`. Coverage audit passed for all maintained target folders under that rule, markdownlint and codespell passed on the touched README files, `mkdocs build --strict` passed, and full `make ci` (`3319 passed`) plus `uv lock --check` passed before closure |
| 9.4 Deployment/operator/contributor docs rewrite | done | `docs: rewrite deployment and contributor guides` | Rewrote the contributor, operator, and deployment how-to surfaces so they now match the final built-in deployment contract and real service/runtime shape instead of carrying partial pre-redesign guidance. `CONTRIBUTING.md` now codifies the repo's real branch/gate/documentation expectations; development setup/install docs now point at the current `uv`/`make install` workflow and required verification steps; Docker/manual deployment, monitoring, backup/restore, troubleshooting, and first-deployment guides now include the full runtime shape (`ranker`, `assertor`, Alertmanager, `DB_RANKER_PASSWORD`, `RANKER_METRICS_PORT`), honest manual role/grant guidance, and restore instructions for the Ranker's private store after point-in-time rollback. Closure audit also fixed the tranche-summary drift in this ledger so Tranches 8 and 9 now reflect the real program state. Targeted `markdownlint`/`codespell`, `mkdocs build --strict`, full `make ci`, and `uv lock --check` all passed before closure |
| 9.5 Final reference alignment pass | done | `docs: align root reference surfaces` | Performed the final root-reference sweep so the generated API reference story, in-code documentation, root current-state guides, and long-form repository/protocol references now tell the same architectural story. `PROJECT_GUIDE.md` was rewritten as an implementation-aligned guide for the final runtime, shared data model, read core, deployment contract, and verification workflow; `README.md`, `AGENTS.md`, and local contributor guidance were aligned to the same four-question product identity and public readable-resource vocabulary; and the long-form support references (`PROJECT_VISION_AND_REDESIGN_PLAN.md`, `BIGBROTR_REPOSITORY_BIBLE.md`, `NOSTR_NIPS_DEEP_ANALYSIS.md`) were updated to remove the remaining split-brain around `metadata` vs `document`, `rank snapshots` vs public scores, and raw `read model` framing vs the readable-resource/`ReadCore` architecture. Targeted `markdownlint`/`codespell`, `mkdocs build --strict`, full `make ci`, and `uv lock --check` all passed before closure |
...
| 10.3 Final repository-wide design-hygiene audit | done | `docs: close final design-hygiene drift` | Performed a repository-wide design-hygiene pass over the live post-redesign surfaces and corrected the last real current-state drift rather than historical planning narrative: `PROJECT_GUIDE.md` no longer claims the removed `ReadModelSurface` wrapper still exists, and the canonical rename ledger now states explicitly that migration-only names such as `ReadModelSurface` / `READ_MODEL_REGISTRY` are gone while `read_model` survives only where it encodes the stable public transport seam. Closure audit also rechecked the active repository surfaces (`src`, `tests`, `docs`, root guides, and the non-historical planning contract) for live references to the removed shared read-side shims and confirmed the remaining mentions are confined to intentional historical execution records. Targeted grep/audit sweeps, targeted `markdownlint`/`codespell`, `mkdocs build --strict`, full `make ci`, and `uv lock --check` all passed before closure |
| 10.4 Final verification and release-readiness gate | done | `chore: close redesign release-readiness gate` | Executed the final release-readiness gate on the completed redesign state: explicit SQL generation verification via `uv run python tools/generate_sql.py --check`; deployment-aware smoke verification that both built-in deployments (`bigbrotr`, `lilbrotr`) still satisfy the normalized deployment contract and can load every registered service class from their shipped YAML config with only dummy DB passwords injected; full `make ci`; and `uv lock --check`. All checks passed on the final repository state before closure, so the redesign execution program now closes with a clean worktree and no remaining mandatory work packages |
