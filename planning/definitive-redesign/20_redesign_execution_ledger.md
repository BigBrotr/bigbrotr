...
| 9.5 Final reference alignment pass | done | `docs: align root reference surfaces` | Performed the final root-reference sweep so the generated API reference story, in-code documentation, root current-state guides, and long-form repository/protocol references now tell the same architectural story. `PROJECT_GUIDE.md` was rewritten as an implementation-aligned guide for the final runtime, shared data model, read core, deployment contract, and verification workflow; `README.md`, `AGENTS.md`, and `CLAUDE.md` were aligned to the same four-question product identity and public readable-resource vocabulary; and the long-form support references (`PROJECT_VISION_AND_REDESIGN_PLAN.md`, `BIGBROTR_REPOSITORY_BIBLE.md`, `NOSTR_NIPS_DEEP_ANALYSIS.md`) were updated to remove the remaining split-brain around `metadata` vs `document`, `rank snapshots` vs public scores, and raw `read model` framing vs the readable-resource/`ReadCore` architecture. Targeted `markdownlint`/`codespell`, `mkdocs build --strict`, full `make ci`, and `uv lock --check` all passed before closure |
