...
| 2.4 Service-state contract | done | `test: rebuild service state integration contract` | Added `tests/integration/shared_db/test_service_state_storage.py` and rebuilt the service-state contract around shared builders plus a fresh non-resetting `Brotr` handle: ordered upsert/get semantics, in-place updates, owner/type isolation on shared keys, nested JSON round-trip, selective delete behavior, and state continuity across a new live `Brotr` handle are now proven inside the target `shared_db/` subtree |
