...
| 1. Higher-band harness foundation | in progress | The `tests/system/` and `tests/live_smoke/` bands now exist physically, compose lifecycle plus artifact capture plus stable runtime addressing are closed, and the harness can now generate deterministic project names, port plans, runtime env files, and rewritten compose files that avoid collisions with shipped deployment resources; fault control, observability APIs, and harness self-audit still remain |
