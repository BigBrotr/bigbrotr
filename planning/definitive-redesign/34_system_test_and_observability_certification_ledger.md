...
| Wave | Status | Notes |
|------|--------|-------|
| 0. Freeze the expanded test problem | in progress | The non-unit tracked baseline, the live deployment/monitoring inventory, and the current-vs-required contract matrix are now frozen explicitly; the target taxonomy still needs its own closing slice |
