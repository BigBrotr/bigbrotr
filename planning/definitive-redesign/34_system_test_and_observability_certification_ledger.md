...
| 4. Service system certification | in progress | `Seeder` and `Finder` are now certified on the composed `bigbrotr` stack, covering real seed-file ownership plus real HTTP-source discovery with cooldown persistence, deduplication, DB-side consequences, restart behavior, and clean once-run exits; the remaining eight service slices stay open |
