...
| 4. Service system certification | in progress | `Seeder`, `Finder`, and `Validator` are now certified on the composed `bigbrotr` stack, covering real seed-file ownership, real HTTP-source discovery, and real `wss` relay validation with persisted failure/backoff semantics against live PostgreSQL; the remaining seven service slices stay open |
