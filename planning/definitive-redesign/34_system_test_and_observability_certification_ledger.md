...
| 3. Deployment stack baseline | in progress | Both built-in profiles now reach deterministic baseline readiness with the same runtime-owned local relay path, honest one-shot compose snapshots, corrected shipped health checks, and an explicit compose contract audit for dependency ordering and health probes; the remaining open work in this wave is teardown/restart certification |
