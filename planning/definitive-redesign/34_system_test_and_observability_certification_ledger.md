...
| 3. Deployment stack baseline | in progress | Both built-in profiles now reach deterministic baseline readiness with the same runtime-owned local relay path, honest one-shot compose snapshots, corrected shipped health checks, and real container-log artifacts; the remaining open work in this wave is the explicit health/ordering audit plus teardown/restart certification |
