...
| 3. Deployment stack baseline | in progress | The `bigbrotr` baseline is now certified end-to-end with fresh image rebuilds, honest one-shot compose snapshots, corrected shipped health checks, and real container-log artifacts; `lilbrotr` plus the cross-profile health/teardown audit remain open |
