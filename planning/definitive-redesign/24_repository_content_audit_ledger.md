...
- `2.1` models/utils/NIPs leaf audit, three-hundred-and-fifty-first
  remediation slice:
  - tightened the `services.finder.configs.ApiSourceConfig` URL boundary so
    authored HTTP source URLs now discard default ports from their canonical
    form;
  - closed the drift where semantically identical endpoints like
    `https://api.example.com/path` and `https://api.example.com:443/path`
    could bypass source dedupe and split checkpoint state across distinct
    keys even though they target the same request-effective URL;
...
- `2.1` models/utils/NIPs leaf audit, three-hundred-and-fifty-fifth
  remediation slice:
  - tightened the authored numeric boundary in
    `services.validator.configs.ProcessingConfig` so `interval` now requires a
    canonical numeric type instead of accepting string aliases;
  - closed the drift where payloads like `"3600"`, `"3600.0"`, `"120"`, or
    `"120.5"` could silently become valid failed-candidate retry windows at
    config load time;
  - added paired coverage in `services.test_validator` proving malformed
...
- `2.1` models/utils/NIPs leaf audit, three-hundred-and-sixty-third
  ...
  - added paired coverage in `services.test_refresher` proving malformed
    integer aliases now fail fast both on the leaf processing model and
    through nested `RefresherConfig.processing` parsing.
- `2.1` models/utils/NIPs leaf audit, three-hundred-and-sixty-fourth
  remediation slice:
  - tightened the authored numeric boundary in
    `services.refresher.configs.ProcessingConfig` so `max_duration` now
    requires a canonical numeric type;
  - closed the drift where payloads like `"3600"` or `"120.5"` could
    silently become valid refresher duration budgets at config load time;
  - added paired coverage in `services.test_refresher` proving malformed
    numeric aliases now fail fast both on the leaf processing model and
    through nested `RefresherConfig.processing` parsing.
...
- `2.1` models/utils/NIPs leaf audit, three-hundred-and-sixty-ninth
  remediation slice:
  - tightened the shared authored integer boundary in
    `core.metrics.MetricsConfig` so `port` now requires a canonical `int`
    value;
  - closed the drift where payloads like `"9090"` or `9090.0` could
    silently become valid metrics endpoint ports inside nested service
    configs at load time;
  - added paired coverage in `core.test_metrics` and `services.test_api`
    proving malformed integer aliases now fail fast both on the shared
    metrics config and through a concrete service config that embeds it.
- `2.1` models/utils/NIPs leaf audit, three-hundred-and-seventieth
  remediation slice:
  - tightened the shared authored string boundary in
    `core.metrics.MetricsConfig` so `host` now trims surrounding whitespace
    and rejects blank values;
  - closed the drift where payloads like `"   "` could survive as
    semantically empty metrics bind hosts inside nested service configs at
    load time;
  - added paired coverage in `core.test_metrics` and `services.test_api`
...
- `2.1` models/utils/NIPs leaf audit, three-hundred-and-seventy-ninth
  remediation slice:
  - tightened the shared authored integer boundary in
    `core.pool_config.RetryConfig` so `max_attempts` now requires a
    canonical `int` value;
  - closed the drift where payloads like `"5"` or `5.0` could silently
    become valid retry-attempt budgets at config load time;
  - added paired coverage in `core.test_pool` proving malformed integer
...
- `2.1` models/utils/NIPs leaf audit, three-hundred-and-ninety-first
  ...
  - added paired coverage in `services.test_ranker` proving
    ...
    through nested `RankerConfig` parsing.
- `2.1` models/utils/NIPs leaf audit, three-hundred-and-ninety-second
  remediation slice:
  - tightened the authored integer boundary in
    `services.ranker.configs.RankerSyncConfig` so `batch_size` now
    requires a canonical integer value;
  - closed the drift where authored aliases like `"1000"` or `1000.0`
    could silently change the follow-graph sync batch budget at config
    load time;
  - added paired coverage in `services.test_ranker` proving
...
- `2.1` models/utils/NIPs leaf audit, three-hundred-and-ninety-third
  ...
  - added paired coverage in `services.test_ranker` proving
    ...
    through nested `RankerConfig` parsing.
- `2.1` models/utils/NIPs leaf audit, three-hundred-and-ninety-fourth
  remediation slice:
  - tightened the authored integer boundary in
    `services.ranker.configs.RankerSyncConfig` so
    `max_followers_per_cycle` now requires a canonical integer value or
    `None`;
  - closed the drift where authored aliases like `"750"` or `750.0`
    could silently change the per-cycle follow-graph sync cap at config
    load time;
  - added paired coverage in `services.test_ranker` proving
...
- `2.1` models/utils/NIPs leaf audit, three-hundred-and-ninety-sixth
  ...
  - added paired coverage in `services.test_ranker` proving
    ...
    and through nested `RankerConfig` parsing.
- `2.1` models/utils/NIPs leaf audit, three-hundred-and-ninety-seventh
  remediation slice:
  - tightened the authored integer boundary in
    `services.ranker.configs.RankerFactsStageConfig` so
    `max_addressable_rows` now requires a canonical integer value or
    `None`;
  - closed the drift where authored aliases like `"2000"` or `2000.0`
    could silently change the addressable fact staging cap at config
    load time;
  - added paired coverage in `services.test_ranker` proving
...
- `2.1` models/utils/NIPs leaf audit, three-hundred-and-ninety-ninth
  ...
  - added paired coverage in `services.test_ranker` proving
    ...
    through nested `RankerConfig` parsing.
- `2.1` models/utils/NIPs leaf audit, four-hundredth remediation slice:
  - tightened the authored integer boundary in
    `services.ranker.configs.RankerExportConfig` so
    `max_batches_per_subject` now requires a canonical integer value or
    `None`;
  - closed the drift where authored aliases like `"4"` or `4.0` could
    silently change the per-subject export batch cap at config load
    time;
  - added paired coverage in `services.test_ranker` proving
...
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-sixth remediation
  ...
  - added paired coverage in `services.test_assertor` proving
    ...
    through root-level `AssertorConfig` parsing.
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-seventh
  remediation slice:
  - tightened the authored string boundary in
    `services.assertor.configs.TrustedProviderListConfig` so `content`
    now requires a canonical `str` before event payload use;
  - closed the drift where authored aliases like `b"hello"` could
    silently decode into valid trusted-provider list content at config
    load time;
...
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-twenty-fourth
  remediation slice:
  - tightened the authored mapping boundary in
    `services.common.configs.ReadModelPolicy` so raw config payloads now
    require canonical string keys before exposure-policy field parsing;
  - closed the drift where authored field keys like `b"enabled"` could
    be ignored silently at config load time, leaving the default
    disabled policy in place instead of exposing the intended readable
    resource;
  - added paired coverage in `services.common.test_configs` and
...
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-thirty-first
  ...
  - added paired coverage in `services.test_finder` proving those raw
    ...
    construction and through root-level `FinderConfig` parsing.
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-thirty-third
  remediation slice:
  - tightened the authored mapping boundary in
...
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-thirty-fourth
  remediation slice:
  - tightened the authored mapping boundary in
    `services.monitor.configs.PublishingConfig` so raw config payloads
    now require canonical string keys before publishing relay field
    parsing;
  - closed the drift where authored field keys like `b"relays"` could
    be ignored silently at config load time, leaving the default
    publishing relays in place instead of applying the intended
    overlay;
  - added paired coverage in `services.test_monitor` proving those raw
    key boundaries now fail fast both on direct `PublishingConfig`
    construction and through root-level `MonitorConfig` parsing.
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-thirty-fifth
  remediation slice:
  - tightened the authored mapping boundary in
    `services.monitor.configs.DiscoveryConfig` so raw config payloads
    now require canonical string keys before discovery field parsing;
  - closed the drift where authored field keys like `b"enabled"` could
    be ignored silently at config load time, leaving discovery
    publishing enabled instead of applying the intended overlay;
  - added paired coverage in `services.test_monitor` proving those raw
    key boundaries now fail fast both on direct `DiscoveryConfig`
    construction and through root-level `MonitorConfig` parsing.
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-thirty-sixth
  remediation slice:
  - tightened the authored mapping boundary in
    `services.monitor.configs.AnnouncementConfig` so raw config payloads
    now require canonical string keys before announcement field parsing;
  - closed the drift where authored field keys like `b"enabled"` could
    be ignored silently at config load time, leaving announcement
    publishing enabled instead of applying the intended overlay;
  - added paired coverage in `services.test_monitor` proving those raw
    key boundaries now fail fast both on direct `AnnouncementConfig`
    construction and through root-level `MonitorConfig` parsing.
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-thirty-seventh
  remediation slice:
  - tightened the authored mapping boundary in
    `services.monitor.configs.ProfileConfig` so raw config payloads now
    require canonical string keys before profile field parsing;
  - closed the drift where authored field keys like `b"enabled"` could
    be ignored silently at config load time, leaving profile publishing
    enabled instead of applying the intended overlay;
  - added paired coverage in `services.test_monitor` proving those raw
...
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-thirty-ninth
  ...
  - added paired coverage in `services.test_monitor` proving those raw
    ...
    construction and through root-level `MonitorConfig` parsing.
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-fortieth
  remediation slice:
  - tightened the authored mapping boundary in
    `services.monitor.configs.RetriesConfig` so raw config payloads now
    require canonical string keys before per-check retry field parsing;
  - closed the drift where authored field keys like `b"nip11_info"`
    could be ignored silently at config load time, leaving retry
...
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-forty-first
  ...
  - added paired coverage in `services.test_monitor` proving those raw
    ...
    construction and through root-level `MonitorConfig` parsing.
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-forty-second
  remediation slice:
  - tightened the authored mapping boundary in
    `services.monitor.configs.MonitorConfig` so raw config payloads now
    require canonical string keys before root-level field parsing;
  - closed the drift where authored field keys like `b"processing"`
    could be ignored silently at config load time, leaving monitor
    subconfigs at defaults instead of applying the intended overlay;
...
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-forty-fifth
  ...
  - added paired coverage in `core.test_brotr` proving those raw key
    ...
    construction and through nested `BrotrConfig` parsing.
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-forty-sixth
  remediation slice:
  - tightened the shared mapping boundary in
    `core.brotr_config.BrotrConfig` so raw config payloads now require
    canonical string keys before root-level field parsing;
  - closed the drift where authored field keys like `b"batch"` could
    be ignored silently at config load time, leaving Brotr subconfigs
...
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-fifty-sixth
  remediation slice:
  - tightened the authored mapping boundary in
    `services.synchronizer.configs.TimeoutsConfig` so raw config
    payloads now require canonical string keys before timeout field
    parsing;
  - closed the drift where authored field keys like `b"idle"` could be
    ignored silently at config load time, leaving synchronizer timeout
    settings at defaults instead of applying the intended overlay;
  - added paired coverage in `services.test_synchronizer` proving those
    raw key boundaries now fail fast both on direct
    `TimeoutsConfig` construction and through nested
    `SynchronizerConfig` parsing.
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-fifty-seventh
  remediation slice:
  - tightened the authored mapping boundary in
    `services.synchronizer.configs.ProcessingConfig` so raw config
    payloads now require canonical string keys before processing field
    parsing;
  - closed the drift where authored field keys like `b"limit"` could
    be ignored silently at config load time, leaving synchronizer
    processing settings at defaults instead of applying the intended
    overlay;
  - added paired coverage in `services.test_synchronizer` proving those
    raw key boundaries now fail fast both on direct
    `ProcessingConfig` construction and through nested
    `SynchronizerConfig` parsing.
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-fifty-eighth
  remediation slice:
  - tightened the authored mapping boundary in
    `services.synchronizer.configs.SynchronizerConfig` so raw config
    payloads now require canonical string keys before root-level field
    parsing;
  - closed the drift where authored field keys like `b"timeouts"`
    could be ignored silently at config load time, leaving
    synchronizer subconfigs at defaults instead of applying the
    intended overlay;
...
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-sixtieth
  remediation slice:
  - tightened the authored mapping boundary in
    `services.refresher.configs.RefresherConfig` so raw config payloads
    now require canonical string keys before root-level field parsing;
  - closed the drift where authored field keys like `b"processing"`
    could be ignored silently at config load time, leaving refresher
    subconfigs at defaults instead of applying the intended overlay;
...
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-sixty-seventh
  remediation slice:
  - tightened the authored field-name boundary in
    `services.synchronizer.configs.ProcessingConfig` so unknown
    processing keys are now rejected instead of being ignored silently
    at config load time;
  - closed the drift where stale payloads like `max_relays: 100` could
    be accepted while leaving synchronizer processing settings at
    defaults instead of applying the intended overlay;
...
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-sixty-ninth
  ...
  - added coverage in `services.test_synchronizer` proving those
    ...
    `SynchronizerConfig` parsing.
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-seventieth
  remediation slice:
  - tightened the authored field-name boundary in
    `services.refresher.configs.ProcessingConfig` so unknown
    processing keys are now rejected instead of being ignored silently
    at config load time;
  - closed the drift where stale payloads like `sync_window: 3600`
    could be accepted while leaving refresher processing settings at
    defaults instead of applying the intended overlay;
  - added paired coverage in `services.test_refresher` proving those
    unknown field names now fail fast both on direct
    `ProcessingConfig` parsing and through nested
...
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-seventy-first
  ...
  - added coverage in `services.test_refresher` proving those unknown
    ...
    parsing.
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-seventy-second
  remediation slice:
  - tightened the authored field-name boundary in
    `services.validator.configs.ProcessingConfig` so unknown
    processing keys are now rejected instead of being ignored silently
    at config load time;
  - closed the drift where stale payloads like `max_relays: 5` could
    be accepted while leaving validator processing settings at
    defaults instead of applying the intended overlay;
  - added paired coverage in `services.test_validator` proving those
    unknown field names now fail fast both on direct
    `ProcessingConfig` parsing and through nested
    `ValidatorConfig` parsing.
- `2.1` models/utils/NIPs leaf audit, four-hundred-and-seventy-third
  remediation slice:
  - tightened the authored field-name boundary in
    `services.validator.configs.CleanupConfig` so unknown cleanup keys
    are now rejected instead of being ignored silently at config load
    time;
  - closed the drift where stale payloads like `retry_limit: 5` could
    be accepted while leaving validator cleanup settings at defaults
    instead of applying the intended overlay;
  - added paired coverage in `services.test_validator` proving those
