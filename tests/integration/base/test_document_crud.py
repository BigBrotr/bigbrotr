...
class TestDocumentInsert:
    ...
    async def test_all_document_types(self, brotr: Brotr, metadata_type: MetadataType) -> None:
        metadata = Document(type=metadata_type, data={"test_key": metadata_type.value})
        inserted = await brotr.insert_document([metadata])
        assert inserted == 1
