"""Integration tests for materialized view refresh and query correctness.

Tests exercise all 7 materialized views and the all_statistics_refresh
meta-function. Pattern: insert data → refresh → query view → verify.
"""
...
class TestAllStatisticsRefresh:
    async def test_refreshes_all_views(self, brotr: Brotr):
        # Insert some data so views are non-trivial
        ...
        # Refresh all views via the meta-function
        await brotr.execute("SELECT all_statistics_refresh()")

        # Verify all views are queryable with data
