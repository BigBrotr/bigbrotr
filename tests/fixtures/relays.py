...
def relay_clearnet_with_port() -> Relay:
    ...
    return Relay("wss://relay.example.com:8443", stored_at=1700000000)


@pytest.fixture
def relay_tor() -> Relay:
    """Tor (.onion) relay with valid 56-char v3 address."""
    return Relay(f"ws://{ONION_HOST}.onion", stored_at=1700000000)


@pytest.fixture
def relay_i2p() -> Relay:
    """I2P (.i2p) relay."""
    return Relay("ws://example.i2p", stored_at=1700000000)


@pytest.fixture
def relay_loki() -> Relay:
    """Lokinet (.loki) relay."""
    return Relay(f"ws://{LOKI_HOST}.loki", stored_at=1700000000)


@pytest.fixture
def relay_ipv6() -> Relay:
    """IPv6 relay with explicit port."""
    return Relay("wss://[2607:f8b0:4000::1]:8080", stored_at=1700000000)
...
def relay_overlay(request: pytest.FixtureRequest) -> Relay:
    ...
    return Relay(urls[request.param], stored_at=1700000000)
