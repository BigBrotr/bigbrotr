"""Unit tests for the Document model and MetadataType enum."""
...
from bigbrotr.models.document import Document, DocumentDbParams, MetadataType
...
class TestConstruction:
    ...
    def test_with_dict(self):
        m = Document(type=MetadataType.NIP11_INFO, data={"name": "test", "value": 123})
    ...
    def test_with_none_rejected(self):
        with pytest.raises(TypeError, match="data must be a Mapping"):
            Document(type=MetadataType.NIP11_INFO, data=None)  # type: ignore[arg-type]

    def test_without_args(self):
        m = Document(type=MetadataType.NIP11_INFO)
        assert m.data == {}

    def test_with_empty_dict(self):
        m = Document(type=MetadataType.NIP11_INFO, data={})
        assert m.data == {}

    def test_custom_type_allowed(self):
        m = Document(type="custom_metadata_type", data={"name": "test"})
        assert m.type == "custom_metadata_type"

    def test_with_nested(self):
        m = Document(type=MetadataType.NIP11_INFO, data={"outer": {"inner": "value"}})
    ...
    def test_with_mixed_types(self):
        ...
        m = Document(type=MetadataType.NIP11_INFO, data=data)
...
class TestContentHash:
    ...
    def test_returns_32_bytes(self):
        m = Document(type=MetadataType.NIP11_INFO, data={"name": "test"})
        assert isinstance(m.content_hash, bytes)
        assert len(m.content_hash) == 32

    def test_deterministic(self):
        m1 = Document(type=MetadataType.NIP11_INFO, data={"name": "test"})
        m2 = Document(type=MetadataType.NIP11_INFO, data={"name": "test"})
        assert m1.content_hash == m2.content_hash

    def test_different_data_different_hash(self):
        m1 = Document(type=MetadataType.NIP11_INFO, data={"name": "test1"})
        m2 = Document(type=MetadataType.NIP11_INFO, data={"name": "test2"})
        assert m1.content_hash != m2.content_hash

    def test_type_not_included_in_hash(self):
        m1 = Document(type=MetadataType.NIP11_INFO, data={"name": "test"})
        m2 = Document(type=MetadataType.NIP66_RTT, data={"name": "test"})
        assert m1.content_hash == m2.content_hash
...
class TestCanonicalJson:
    ...
    def test_sorted_keys(self):
        m = Document(type=MetadataType.NIP11_INFO, data={"z": 1, "a": 2})
    ...
    def test_unicode_preserved(self):
        m = Document(type=MetadataType.NIP11_INFO, data={"name": "Relay"})
...
class TestImmutability:
    ...
    def test_attribute_mutation_blocked(self):
        m = Document(type=MetadataType.NIP11_INFO, data={"key": "value"})
...
class TestSanitize:
    ...
    def test_non_string_keys_rejected(self):
        with pytest.raises(TypeError, match="data keys must be str, got int"):
            Document(type=MetadataType.NIP11_INFO, data={1: "value", "key": "value"})
    ...
    def test_non_serializable_value_rejected(self):
        ...
        with pytest.raises(TypeError, match="data contains unsupported type Custom"):
            Document(type=MetadataType.NIP11_INFO, data={"valid": "ok", "invalid": Custom()})

    def test_non_finite_float_rejected(self):
        with pytest.raises(ValueError, match="data contains a non-finite float"):
            Document(type=MetadataType.NIP11_INFO, data={"value": float("inf")})

    def test_null_bytes_rejected(self):
        with pytest.raises(ValueError, match="null bytes"):
            Document(type=MetadataType.NIP11_INFO, data={"text": "hello\x00world"})

    def test_deeply_nested_normalization(self):
        m = Document(type=MetadataType.NIP11_INFO, data={"l1": {"l2": {"l3": {"l4": "value"}}}})
...
class TestToDbParams:
    ...
    def test_nested(self):
        m = Document(type=MetadataType.NIP11_INFO, data={"a": {"b": {"c": [1, 2, 3]}}})
        params = m.to_db_params()
        parsed = json.loads(params.data)
...
class TestEquality:
    ...
    def test_equal(self):
        m1 = Document(type=MetadataType.NIP11_INFO, data={"key": "value"})
        m2 = Document(type=MetadataType.NIP11_INFO, data={"key": "value"})
        assert m1 == m2

    def test_different_value(self):
        m1 = Document(type=MetadataType.NIP11_INFO, data={"key": "value1"})
        m2 = Document(type=MetadataType.NIP11_INFO, data={"key": "value2"})
        assert m1 != m2

    def test_different_type(self):
        m1 = Document(type=MetadataType.NIP11_INFO, data={"key": "value"})
        m2 = Document(type=MetadataType.NIP66_RTT, data={"key": "value"})
        assert m1 != m2
...
class TestEdgeCases:
    ...
    def test_list_as_data_rejected(self):
        with pytest.raises(TypeError, match="data must be a Mapping"):
            Document(type=MetadataType.NIP11_INFO, data=[1, 2, 3])

    def test_string_as_data_rejected(self):
        with pytest.raises(TypeError, match="data must be a Mapping"):
            Document(type=MetadataType.NIP11_INFO, data="not a dict")
...
class TestNormalization:
    ...
    def test_empty_lists_preserved(self):
        m = Document(type=MetadataType.NIP11_INFO, data={"name": "Relay", "tags": [], "nips": []})
        assert m.data == {"name": "Relay", "nips": (), "tags": ()}

    def test_nested_empty_preserved_recursively(self):
        m = Document(type=MetadataType.NIP11_INFO, data={"level1": {"level2": {"level3": {}}}})
    ...
    def test_keys_sorted(self):
        m1 = Document(type=MetadataType.NIP11_INFO, data={"z": 1, "a": 2, "m": 3})
        m2 = Document(type=MetadataType.NIP11_INFO, data={"a": 2, "m": 3, "z": 1})
        assert m1.to_db_params().data == m2.to_db_params().data

    def test_hash_distinguishes_empty_containers(self):
        m1 = Document(type=MetadataType.NIP11_INFO, data={"name": "Relay", "limitation": {}})
        m2 = Document(type=MetadataType.NIP11_INFO, data={"name": "Relay"})
        assert m1.content_hash != m2.content_hash
    ...
    def test_empty_in_lists_preserved(self):
        m = Document(type=MetadataType.NIP11_INFO, data={"items": [1, [], 2, {}, 3]})
        assert m.data == {"items": (1, (), 2, {}, 3)}

    def test_none_in_lists_preserved(self):
        m = Document(type=MetadataType.NIP11_INFO, data={"items": [1, None, 2, None, 3]})
        assert m.data == {"items": (1, None, 2, None, 3)}

    def test_falsy_values_preserved(self):
        m = Document(type=MetadataType.NIP11_INFO, data={"enabled": False, "count": 0, "name": ""})
