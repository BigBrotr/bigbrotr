...
The stable transport still uses ``read_model`` in discovery payloads and
response metadata, but the handler logic itself is resource-oriented and
delegates into the shared
...
class ApiReadableResourceHandler:
    ...
    def primary_key_columns(self) -> tuple[str, ...]:
        ...
        return self.resource.schema(self.read_core.catalog).primary_key
    ...
    async def list_rows(self, request: Request) -> JSONResponse:
        ...
        try:
            result = await asyncio.wait_for(
                self.read_core.query_resource(self.brotr, self.resource, query),
        ...
        return JSONResponse(
            {
                "data": result.rows,
                "meta": build_read_model_meta(result, read_model_id=self.resource_id),
            }
        )
    ...
    async def get_row(self, request: Request) -> JSONResponse:
        ...
        try:
            row = await asyncio.wait_for(
                self.read_core.get_resource_by_pk(self.brotr, self.resource, pk_values),
                timeout=self.request_timeout,
            )
        except TimeoutError:
            return JSONResponse({"error": "Query timeout"}, status_code=504)
