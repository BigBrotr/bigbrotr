...
LEFT JOIN LATERAL (
    SELECT
        ROUND(AVG((m.payload ->> 'rtt_open')::INTEGER)::NUMERIC, 2) AS avg_rtt_open,
        ROUND(AVG((m.payload ->> 'rtt_read')::INTEGER)::NUMERIC, 2) AS avg_rtt_read,
        ROUND(AVG((m.payload ->> 'rtt_write')::INTEGER)::NUMERIC, 2) AS avg_rtt_write
    FROM (
